...
function SuperAgent() {
	...
	useEffect(() => {
		axios.get("/getAllSuperAgents").then((res) => {
			setDatas(res.data);
			res.data.map((agent) =>
				agent.super_agents.map((super_id) =>
					axios
						.get(`/getSuperAgentById?id=${super_id}`)
						.then((res) =>
							setSupers((supers) => [...supers, res.data]),
						),
				),
			);
		});
	...
	return (
		<>
			<Helmet>
				<title>সুপার এজেন্ট লিষ্ট - Velki Agents</title>
			</Helmet>
					...
					{datas?.map((data) => (
						<div key={data._id}>
							...
							<h1 className="text-sm md:text-xl text-center font-semibold text-[#000000c5] my-2 mt-4">
								সাব এডমিন {data.agent_id} এর অধীনে সুপার এজেন্ট{" "}
								{data.super_agents.length} জন
							</h1>
